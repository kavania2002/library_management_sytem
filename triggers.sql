...
CREATE DEFINER=`root`@`localhost` TRIGGER `check_copies` AFTER INSERT ON `dates` FOR EACH ROW BEGIN
    ...
    set no_of_copies = (select `admin`.copies from `admin`,`book` where `book`.title = book_name);
...
CREATE DEFINER=`root`@`localhost` TRIGGER `check_number` BEFORE INSERT ON `reader` FOR EACH ROW BEGIN
	...
	-- set cnt = (select new.`contact_no` from `reader`);
	set cnt = 0;
...
-- Email verification
DROP TRIGGER IF EXISTS `lms`.`check_email_before`;
...
DROP TRIGGER IF EXISTS `lms`.`check_email_update`;
...
DROP TRIGGER IF EXISTS `lms`.`check_price_insert`;
DELIMITER $$
USE `lms`$$
...
BEGIN
    ...
    if p < 0 then
		set error_msg = `Price cannot be negative`;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
	end if;
END$$
DELIMITER ;
